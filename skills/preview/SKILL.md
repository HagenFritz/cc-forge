...
4. **Check for dependency/migration drift before merging anything.**
   - Diff the target branch against the freshly-fetched default branch for manifest and migration files: `git diff origin/<default-branch>...<branch> --name-only` and check the result against common patterns — `package.json`, `pnpm-lock.yaml`, `package-lock.json`, `yarn.lock`, `pyproject.toml`, `uv.lock`, `poetry.lock`, `requirements*.txt`, and any path containing `migrations/` or `alembic/`.
...
5. **Create the preview branch and merge.**
   ...
   - `git merge --no-ff <branch>`. This is a real commit on the disposable `preview/*` branch — never touches `<default-branch>` or the worktree's own branch.
