...
| 8 / 8 | 37 | 49 | 73 |
| 16 / 20 | 45 | 69 | 93 |
| 24 / 30 (both caps) | 53 | 87 | 111 |

The bottom row is the worst case. Each threshold is 2 higher than it was before the selection gutter, which is exactly what the gutter costs: `buildTable` lays out against `frameWidth - GUTTER_WIDTH`, so the two columns come off the budget once, at the top, rather than being spent per column. SKILL caps at 14 because 20 pushed that worst case past the 122-column panes this runs in; it would now fit either way, but 14 is what is measured here and the cap stays. Every real terminal here (122–281 columns) keeps all seven columns, with 11 to spare in the worst case.

`COLUMN_GAP` is 2, not the 4 it shipped with. At 4, plus the per-column padding each field already carries, neighbouring columns sat 10 to 24 display columns apart and the table read as scattered islands rather than a grid. The columns that gap gave back went to SUMMARY, which is the only column that was ever short of room and the one that absorbs whatever the fixed budgets leave. What it actually gets depends entirely on how wide the frame's own names and dirs are: on a 122-wide frame it measures 61 columns against a short name and dir, and 21 against both caps. The selection gutter takes 2 from whichever of those applies. `AGE_WIDTH` is 5 for the same reason — `formatAge` maxes at `1h00`, so 6 was padding a 4-character worst case.
...
The listener is an event stream, and a session the stream never mentions is invisible to it: one started before its hooks loaded, one started before the dashboard was up, one whose emitter is silently failing. All three happened at once and read as "the VM row never appears." So the devbox is also **asked directly**, every 10 seconds and once immediately at startup, with the first poll going out before the first frame:

```
ssh ro-devbox bash -lc 'claude agents --json; echo ---TMUX---; tmux list-panes -a -F "#{session_name} #{pane_tty}"; echo ---PS---; ps -eo pid=,tty='
```

One round trip, three sections. The registry JSON is the same `claude agents --json` the local table is built from. The tmux and `ps` sections give each session its tmux name by the same tty join the local tab order uses: session pid → tty → the pane on that tty. That name is what Enter switches to, and the emitter only ever carried it when `$TMUX` was set in the hook's environment. `bash -lc` because a non-interactive ssh shell has no `~/.local/bin` on PATH and so no `claude`. The rows go through `validateRows` as `remote`, the same door as every emitter payload.

Merge rule, in `applyVmPoll`: the listing is authoritative for **which sessions exist**; the emitter stays authoritative for **status** only inside its own 5 s sticky window, because a `waiting` reaches the stream a beat before the registry file catches up. A polled row carries `seq: -1`, below every real seq, so the first emitter event for that session is never dropped as a replay. A row the listing no longer names is removed once it has been quiet for the sticky window, so a session mid-registration is not deleted by the poll that raced its first event. `SessionEnd` still sticks: a polled row inside the end window is skipped.

Own timer, not every Nth tick: a slow IAP tunnel must never hold the two-second paint loop. `ConnectTimeout=8` rather than the focus path's 2, since when no devbox connection exists this poll is the one that establishes the ssh master through the tunnel. One poll in flight at a time. A failed poll keeps every held row — they age out on the normal schedule — and the footer says `ro-devbox unreachable` only after three consecutive failures, so one slow tunnel is not a warning. A reply that does not parse gets its own footer line: that is a changed `claude agents --json` shape, not a network problem, and needs a person. Gated with the listener on `--no-listen`, since that flag means there is no devbox to ask.
...
- Enter — focus the highlighted session's iTerm tab. On a VM row there is no pid
  to resolve, so focus finds the devbox tab the way local rows are found: the
  tty of the local `ssh ro-devbox` process (`ps -Ao pid=,tty=,command=`, first
  argv token `ssh`, host a whole token after it), then the tab on that tty. The
  tab's title is irrelevant — it used to be matched on `ro-devbox` and never
  was, because tmux and the prompt repaint it (`devbox (ssh)` is what it reads).
  The title match survives only as the fallback for an ssh `ps` cannot see. Then
  it asks `tmux list-clients` over ssh for the attached client — taking the first
  token of each line, because ssh joins its argv into one remote command line and
  a bare `#{client_tty}` then starts a shell comment, so `-F` gets no format and
  tmux prints its default `/dev/pts/0: tasks [...]` — and runs
...
- `r` — rename the highlighted session (inline prompt; Enter confirms, Esc cancels, `^U` clears). Two writes, in order: the Claude Code session name, then the iTerm tab title. See *Rename* below.
- `q` / `^C` — quit.
...
`r` renames **two** things: the Claude Code session, then the iTerm tab. It used to rename only the tab, which made it look broken — the NAME column never moved, because the column and the tab title are unrelated values. NAME renders what `claude agents --json` reports, and that comes from `~/.claude/sessions/<pid>.json`; no amount of renaming an iTerm tab touches it.

There is no supported way to rename a running session from outside. `rename_session` exists as a real verb in the binary, but on the **SDK control channel** (`onRenameSession`, a `control_response` envelope), which exists only when Claude Code is driven by a parent process. A terminal session exposes no such channel, and the peer socket at `/tmp/cc-socks/<pid>.sock` advertises `notify_idle`, `reply_across_default_dirs`, and `artifact_yield` — messaging, no rename.

So the registry file is written directly, matching what Claude Code's own handler writes: `name`, plus `nameSource: "user"`. That second field is load-bearing rather than incidental — a `derived` name is recomputed by the session, so writing the name alone would be reverted. Verified live: a renamed session survives subsequent turns and reports the new name through `claude agents --json` within one poll.

This is another program's private state, so **every guard is a refusal to write**, never a repair. The file must already exist, be a regular non-symlink file under the size cap, parse as an object, and already carry string `name` and `nameSource` fields — anything else is left untouched and reported in the footer. A schema change in a future Claude Code version therefore degrades to "rename does nothing, and says so" rather than to a corrupted registry. The write is temp+rename at the file's existing mode, so a crash mid-write cannot leave a half-written file for the session process to read.

The session rename runs **first and independently** of the tab rename: iTerm being absent or unreachable must not cost the rename the user can actually see in the table. The row's name is also updated in place so the change shows in the current frame rather than after the next poll — that poll is what makes it durable.

## Selection marker

The highlighted row is marked with `> ` in a two-column gutter that every table line carries, including the header, the agent lines, and the `+N more` notice.

It was reverse video, and that could not work here: **a row is two frame lines**, so inverting the single line `rowLineIndex` points at lit up the top half of a selection and left the summary continuation below it plain. Widening the invert to both lines would have traded that for a worse problem — STATE is already green, amber, or dim inside that same span, and inverting an already-coloured cell reads as a rendering fault rather than as a selection. A gutter sits outside every column, so it marks the row without touching what the row is painted.

Two ordering rules, both the same shape as the existing colour rules:

- **The gutter is prepended after `recolorSpan`**, never before. Those offsets are column-relative, and a two-character prefix would shift every one of them.
- **`buildTable` lays out against `frameWidth - GUTTER_WIDTH`**, so the columns it sizes still fit once the gutter is on. The marker then overwrites the reserved blank rather than being inserted, which is what keeps every line its original width and every column under its own header.
...
- A VM session that starts while the dashboard is down appears on the next poll, within 10 s (see *VM poll*); it no longer waits for an event. `stale` after 10 minutes therefore means the poll has been failing that long as well as the stream being quiet — the footer's `unreachable` line will say so.
...
- `r` on a VM row reports that rename is local-only: a VM row has no pid, so there is no local registry file to write and no local tab to retitle.
...
- The **tab title** half of `r` is overwritten by Claude Code's own OSC 0 title on that session's next turn — not sticky. Mitigation is the iTerm profile toggle "Terminal may set tab/window title"; there is no scriptable lock. The **session name** half is unaffected and survives, which is what the NAME column reads.
- Renaming writes `~/.claude/sessions/<pid>.json`, which Claude Code owns and this program only borrows. The field names are read out of a shipped binary, not a published contract, so a version bump could change them; the guards turn that into a refusal with a footer message rather than a bad write. If `r` starts reporting "session registry has no name field", that is this bet coming due.
...
- One devbox tab is assumed: focus takes the tty of the first `ssh ro-devbox` process `ps` lists, so with two such tabs open it may pick either. The tmux switch targets the attached client explicitly (`-c <client_tty>`, the first one `list-clients` reports), so it is not iTerm's pick that decides which session moves — but with two attached clients the tab iTerm selects and the client tmux moves can still be different ones.
